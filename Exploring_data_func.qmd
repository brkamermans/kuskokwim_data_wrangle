...
escapement <- read_csv("./Data/ADFG_test/data_tidy_escapement_allYrs.csv") %>%
  mutate(date = case_when(
    river == "Takotna River" ~ as.Date(date, format = "%Y-%m-%d"),
    .default = as.Date(date, format = "%m/%d/%y")))
```
...
```{r}
#for george
escapement.george.clean <- george.esc %>%
# select columns we want to keep
select(river, year, mon.day, date, fish.count, species)

#for kwethluk
escapement.kwethluk.clean <- kwethluk.esc %>%
# select columns we want to keep
select(river, year, mon.day, date, fish.count, species)

#for takotna
escapement.takotna.clean <- takotna.esc %>%
# select columns we want to keep
select(river, year, mon.day, date, fish.count, species)
```
...
```{r}
#for george
george.esc.2023 <- escapement.george.clean %>%
   filter(year > 2022)

#for kwethluk
kwethluk.esc.2023 <- escapement.kwethluk.clean %>%
   filter(year > 2022)

#for takotna
takotna.esc.2023 <- escapement.takotna.clean %>%
   filter(year > 2022)
```
...
```{r}
plot(george.esc.2023$date, george.esc.2023$fish.count)

```

```{r}
plot(kwethluk.esc.2023$date, kwethluk.esc.2023$fish.count)

```

```{r}
plot(takotna.esc.2023$date, takotna.esc.2023$fish.count)
...
```{r}
plot(chinook.chum.george.esc.2023$date, chinook.chum.george.esc.2023$fish.count)
```

```{r}
plot(chinook.chum.kwethluk.esc.2023$date, chinook.chum.kwethluk.esc.2023$fish.count)
```

```{r}
#| echo: false

plot(chinook.chum.takotna.esc.2023$date, chinook.chum.takotna.esc.2023$fish.count)
```
